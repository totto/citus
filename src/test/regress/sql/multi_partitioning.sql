...
-- test foreign keys on partitioned tables
CREATE TABLE partitioning_test_fk(id int, time date) PARTITION BY RANGE (time);

CREATE TABLE partitioning_test_fk_2009 PARTITION OF partitioning_test_fk FOR VALUES FROM ('2009-01-01') TO ('2010-01-01');
CREATE TABLE partitioning_test_fk_2010 PARTITION OF partitioning_test_fk FOR VALUES FROM ('2010-01-01') TO ('2011-01-01');

ALTER TABLE partitioning_test_fk_2010 ADD PRIMARY KEY(id);
ALTER TABLE partitioning_test_fk_2009 ADD CONSTRAINT partition_fk FOREIGN KEY(id) REFERENCES partitioning_test_fk_2010(id);

SELECT create_distributed_table('partitioning_test_fk', 'id');
